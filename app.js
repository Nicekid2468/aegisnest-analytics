...
(() => {
  ...
  function renderTradeLedger(equityCurve) {
    const allTradesNewestFirst = [...equityCurve].reverse();
  ...
  function playEntranceTimeline() {
    ...
    const VISIBLE_ROW_COUNT = 5; // rows visible in the panel before scrolling
    const staggeredRows = Array.from(tradeRows).slice(0, VISIBLE_ROW_COUNT);
    const remainingRows = Array.from(tradeRows).slice(VISIBLE_ROW_COUNT);

    gsap.set(staggeredRows, { autoAlpha: 0, x: -24 });
    gsap.set(remainingRows, { autoAlpha: 1, x: 0 }); // below the fold — no need to animate 340 rows in
    ...
    // Auto-scroll pass through the full 340-trade history — there's no user
    // present during Puppeteer capture, so the list has to scroll itself.
    const maxScroll = dom.tradesList.scrollHeight - dom.tradesList.clientHeight;
    ...
    if (maxScroll > 0) {
      ...
      const scrollDuration = gsap.utils.clamp(14, 30, tradeRows.length * 0.07);
      ...
      tl.to(dom.tradesList, {
        ...
        .to({}, { duration: SCROLL_HOLD_AT_BOTTOM }); // hold on the last row
    }
  }
})();
